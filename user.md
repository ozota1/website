...
|生存期間|必須|ノートブックの生存期間(h)を指定する。立ち上げたノートブックは、生存期間終了後に自動で終了する。|
...
|データセット|任意|利用するデータセットを指定する。ノートブックで利用したデータセットについては、利用後の変更も可能。|
|マウントする学習|任意|学習ジョブの結果をマウントして利用する場合に指定する。複数の学習ジョブをマウント可能。|
|環境変数|任意|学習に渡す環境変数を指定する。環境変数はコンテナ内の環境変数へ追加設定される。|
|パーティション|任意|KAMONOHASHI に設定されているパーティションを設定する。例えばGPUの型番などはここから指定する。|
...
ホームディレクトリは`/kqi`であり、下記ディレクトリをそれぞれの用途で利用できます。

|パス|内容|説明|
|---|---|---|
|/kqi/input/|入力ファイル|指定したデータセットがダウンロードされるディレクトリ|
|/kqi/output/|出力ファイル|出力ファイルを書き出すディレクトリ。ノートブック終了・再起動後も内容が保持される|
|/kqi/git/|モデル|指定したgitリポジトリがcloneされるディレクトリ|
|/kqi/parent/|モデル|マウントする学習で指定したジョブの学習結果がマウントされるディレクトリ|
