...
# 前提
読者がLinux、Gitの基本的な知識を持つことを前提としている。
...
![レジストリトークンの設定](/assets/images/user_setting2.png)
...
|メモリ|任意|ジョブが使用するメモリ量(GB)のデフォルト値を指定する。実行時に再設定が可能。|
|GPU|任意|ジョブが使用するGPU枚数のデフォルト値を指定する。実行時に再設定が可能。|

KAMONOHASHI上で前処理を実行したい場合は、メモを除くすべての項目が必須入力となる。

## 前処理実行

KAMONOHASHIで前処理を実行するためは、以下三つのフローのいずれかで前処理実行画面を表示する。
...
KAMONOHASHIでは後述するジョブを実行する際の入力ファイルは、データセット単位となる。
ジョブ実行時に使用するデータセットを指定すると、training 等の分類に応じてデータが異なるディレクトリにダウンロードされる。
KAMONOHASHIではトレーサビリティを重視しているため、一度ジョブと関連付けられたデータセットは変更できない。
...
## データセットのコピー
KAMONOHASHI では1度ジョブに利用したデータセットは編集できなくなる。このため、過去に利用したデータセットから派生して処理を行い
たい場合には、データセットをコピーして利用する。データセットをコピーするには、一覧からコピーしたいデータセットを選択し、
右上のコピーボタンから行う。

# ジョブ管理
ジョブは KAMONOHASHI が管理するクラスタで計算を実行するための最小単位を表す。
ジョブを起動すると KAMONOHASHI はクラスタから指定されたCPU、メモリ、GPUリソースを確保し、Dockerコンテナを起動して計算環境を用意する。ユーザはこの環境を利用し、任意の計算を行うことができる。

## ジョブの起動
ジョブを起動するには[ジョブ管理]を選択し、右上の新規登録ボタンから行う。[実行コマンドやモデルの仕様については後述](#ジョブの仕様)。
...
|種類|要否|説明|
|---|---|---|
|ジョブ名|必須|ジョブの名前。<br>半角英数小文字、または記号(“-”（ハイフン）、”.”（ドット）) 30 文字以下で指定可能。|
|親ジョブ|任意|既に実行したジョブとの関連付ける場合に指定する。指定時に親ジョブの設定がコピーされるので、変更したい箇所を指定後に変更する。また、親ジョブに指定したジョブの出力結果が、コンテナにマウントされる。|
|データセット|必須|ジョブの入力ファイルとなるデータセットを指定する。|
...
|モデル|必須|ジョブで利用するソースコードを格納しているGitサーバ、リポジトリ、ブランチ、コミットIDを指定する。コミットIDを指定した場合はブランチの入力値は無視される。コミットIDが未指定の場合はブランチのHEADが選択される。ブランチもコミットIDも未指定の場合は、masterブランチのHEADが選択される。|
|実行コマンド|必須|ジョブで実行するコマンドを指定する。コンテナイメージに設定されたcommand, entrypointは無効になり、このコマンドのみが実行される|
...
|種類|要否|説明|
|---|---|---|
|CPU|必須|ジョブが利用するCPUコア数を指定する。|
|メモリ|必須|ジョブが利用するメモリ量(GB)を指定する。|
|GPU|必須|ジョブが利用するGPU枚数を指定する。|
...
|種類|要否|説明|
|---|---|---|
|環境変数|任意|ジョブに渡す環境変数を指定する。環境変数はコンテナ内の環境変数へ追加設定される。|
|パーティション|任意|KAMONOHASHI に設定されているパーティションを設定する。例えばGPUの型番などはここから指定する。|
|メモ|任意|ジョブに付与する任意のメモ。|

## ジョブの確認

過去に実行したジョブの情報・状態・結果などを確認したい場合、[ジョブ管理]の画面で該当ジョブを選択し、ジョブ履歴画面を表示する。

![ジョブ詳細画面](/assets/images/figure8.png)

ジョブ履歴画面では、各種情報が項目毎に一覧化されており、以下のことなどが行える。

### TensorBoard の起動
ジョブを実行しているコンテナの TensorBoard を表示したい場合、[ジョブ管理]の画面で該当ジョブを選択する。
Tensorbardの項目からTensorboadが起動できる。
ただし、TensorBoard に読み込ませるログデータが、ジョブの結果ファイルとして出力されている必要がある。

### ジョブの停止
ジョブを停止したい場合、[ジョブ管理]の画面で該当ジョブを選択する。操作の項目にあるジョブ停止ボタンからジョブを停止することができる。
※この操作はジョブのステータスがRunning時のみ実行可能である。

### ジョブコンテナへの接続
ジョブを実行しているコンテナに接続したい場合、[ジョブ管理]の画面で該当ジョブを選択し、[Shell起動]ボタンを押下する。
コンテナ内のrootユーザとしてシェルが立ち上がり、コンテナ内で任意のコマンドを実行できる。
※この操作はジョブのステータスがRunning時のみ実行可能である。

### コピー実行
類似のジョブを投入したい場合は、過去の該当ジョブを選択し右上のコピー実行ボタンから行える。コピー実行を選択すると、
過去のジョブを投入した情報が初期値として設定され、必要な個所を変更して実行できる。

## ジョブの仕様

### KAMONOHASHIによるジョブの実行フロー

1. クラスタからリソースを確保する
    - リソースに十分な空きがなかった場合は待機する
1. リソース上にコンテナがデプロイされる
    - コンテナ内に環境変数が設定される。
1. 親ジョブの指定があった場合、/kqi/parent に親ジョブの出力結果を読み取り専用でマウントする
1. 学習結果ファイル保存用ディレクトリ /kqi/output を作成する
1. kamonohashiのジョブ制御に必要なツールをインストールする
    - git, zip, python, pip, kamonohashi-cli がコンテナにインストールされる
    - コンテナに既にこれらのツールがインストール済みだった場合はスキップされる
1. モデルを /kqi/model に git clone する
1. データセットを /kqi/input にダウンロードする
1. 指定された「実行コマンド」が実行される
    - /kqi/model がカレントディレクトリになる
    - 実行ユーザはコンテナのrootユーザとなる
    - 学習中、結果ファイルが /kqi/output に書き出されるように「実行コマンド」またはモデルの記述が必要
1. 結果ファイルを保存する
    - /kqi/output をzip圧縮する。このファイルはジョブ履歴画面の添付ファイル「output.zip」からダウンロードできる。
1. 実行結果のステータスが更新される
...
* /kqi/output/配下のファイル名・ディレクトリ名は次の文字だけ使用可
    * -_1-9a-zA-Z/
* /kqi/output に書き込まれたファイルは ジョブ履歴画面の「ファイル一覧」ボタンからリアルタイムに閲覧・ダウンロードできる

#### /kqi/model/ 学習モデル

* 実行時に指定したモデルは /kqi/model に git clone される
* モデルに限らず、コンテナ内で実行したいコマンドがあれば、スクリプトで記述してモデルと共にリポジトリに入れておく
* 実行時のカレントディレクトリはこの /kqi/model/ になる。　
     * 例：リポジトリのトップに「entrypoint.py」を置いた場合、 ジョブ実行の「実行コマンド」には「python entrypoint.py」を記述すればよい

#### /kqi/attach 添付ファイル

* ジョブ履歴画面にて、添付ファイルとしてアップロードされたファイルが格納される

#### /kqi/parent 親ジョブの結果ファイル

* 親ジョブを指定した場合、指定したジョブの出力結果（当該ジョブの /kqi/output に出力されたファイル）が読み取り専用で格納される


### ジョブ実行環境で設定される環境変数

* 下記環境変数がkamonohashiによって設定される
   * 上書き可の環境変数はジョブ新規登録時に環境変数入力フォームから上書き可能
...
|https_proxy|<デプロイ環境ごと>|可|プロキシ設定。システム管理者が指定したデフォルト値が設定される|
|no_proxy|<デプロイ環境ごと>|可|プロキシ設定。システム管理者が指定したデフォルト値が設定される|
|JOB_ID|<ジョブごと>|不可|kamonohashiがジョブの制御に使用|
|DATASET_ID|<ジョブごと>|不可|kamonohashiがジョブの制御に使用|
|PARENT_ID|<ジョブごと>|不可|kamonohashiがジョブの制御に使用|
|MODEL_REPOSITORY|<ジョブごと>|不可|kamonohashiがジョブの制御に使用|
|MODEL_REPOSITORY_TOKEN|<ジョブごと>|不可|kamonohashiがジョブの制御に使用|
|COMMIT_ID|<ジョブごと>|不可|kamonohashiがジョブの制御に使用|
|KQI_SERVER|<ジョブごと>|不可|kamonohashiがジョブの制御に使用|
|KQI_TOKEN|<ジョブごと>|不可|kamonohashiがジョブの制御に使用|
|ENTRY_POINT|<ジョブごと>|不可|kamonohashiがジョブの制御に使用|
