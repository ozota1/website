...
#### ユーザのテナント所属状況の確認
* ユーザ一覧画面から所属テナントが確認できます。
* テナントの色は以下を表します。
  * 青色：デフォルトテナント
  * 黒色：設定画面で紐づけられたテナント
  * 緑色：ユーザグループ経由で紐づけられたテナント

![ユーザ一覧画面(スクショ3-11)](/assets/images/admin_figure11.png)

* 左の「>」を押下すると、テナントごとに付与されているロールを確認することができます。
* ロールの色は以下を表します。
  * 青色：設定画面で紐づけられたロール
  * 緑色：ユーザグループ経由で紐づけられたロール

![ユーザ一覧画面(スクショ3-12)](/assets/images/admin_figure12.png)

#### LDAP同期

認証タイプが LDAP の全ユーザについて、Active Directory(AD) 側で管理されている組織やグループの所属情報と KAMONOHASHI のユーザグループ情報を同期し、所属テナントやロール情報を更新します。

LDAP同期は、以下のフローで実行できます。

1. 管理権限を持つユーザでログイン
1. [システム管理] → [ユーザ管理]を選択し、ユーザ一覧画面を表示する
1. 画面右上の LDAP同期ボタンを押下する
1. AD に登録されているユーザID(sAMAccountName)とパスワードを入力し、同期開始ボタンを押下する
1. 認証タイプが LDAP の全ユーザの所属テナントとロール情報が更新される
...
ユーザグループは Active Directory(AD) の組織(OU)またはグループを表し、テナントに関連付けられます。
...
| 種別                         | 必須 | OU かグループかを選択する。                                                                                |
| DN                           | 必須 | AD で設定されている OU またはグループの DN(distinguishedName) を入力する。                                 |
